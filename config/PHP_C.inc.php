...
class Database
{
    ...
    function connect_db()
    {
        $this->connect = new mysqli(HOST, USER_HOST, USER_PASS, DATABASE);
...
Class Login
{
    ...
    function Check_in_database()
    {
        ...
        $this->query = mysqli_query($this->connections(), "SELECT `id`,`username`,`password` FROM `" . ADMIN_TABLE . "` WHERE `username`='" . $this->user . "' AND password ='" . $this->password . "'") or die(mysql_error());
        if (mysqli_num_rows($this->query) != 0) {
